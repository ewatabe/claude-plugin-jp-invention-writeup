# リッチスライドデザイン — 役員説明レベルの発明説明資料

`scripts/build_explainer_pptx.py`（markdown ベース）の出力は **編集容易だがビジュアル面で簡素** になりがちで、役員審議会・特許事務所打合せでの説得力が物足りない場合がある。
このリファレンスは、`HTML+CSS で個別スライドを記述 → Chrome ヘッドレスで PNG レンダ → PPTX に 1 スライド 1 画像で配置` する **リッチスライド方式** をまとめる。

## いつ採用するか

| 条件 | 推奨方式 |
|---|---|
| 発明者・社内エンジニアレビュー用、後で頻繁に文言修正する | `build_explainer_pptx.py`（markdown） |
| **社内発明審議会・経営陣説明・特許事務所打合せ** | **リッチスライド方式（本書）** |
| 出願書類の図面（参照符号付き、モノクロ） | `build_figure_pptx.py`（Phase 4） |

リッチスライド方式は **ビジュアル品質を最優先** にする代わりに、テキスト編集は HTML を直接編集して再レンダ する手間がある。役員クラスへの説明資料では、品質トレードオフとして合理的。
...
| ファイル | 用途 |
|---|---|
| `work/invention-explainer-draft.md` | 発明者レビュー、本文の言い回し検討 |
| `work/html-slides/*.html` | 役員説明レベルのビジュアル品質を求めるとき |

最初は markdown でドラフト → 内容確定後にリッチスライド化、という二段階運用が無駄が少ない。
