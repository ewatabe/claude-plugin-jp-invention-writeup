...
## 対話ルール：判断ポイントは AskUserQuestion で聞く（最重要）

このスキルは各フェーズを **対話的に** 進める。**発明者に何かを選ばせる／決めさせる局面では、原則として `AskUserQuestion` ツールを使い、具体的な選択肢を提示すること。** 平文テキストで「どうしますか？」と書いて発明者の返答を待つ、という済ませ方をしてはならない。

理由：選択肢を出すと発明者の認知負荷が下がり、論点が明確になり、決定が記録に残る。自由記述だけだと発明者が判断を流してしまい、Claude が推測で埋めてしまう事故（Phase 2〜4 全体に波及する）につながる。

### AskUserQuestion を必ず使う局面（例）

- 開始時チェックリスト（案件名／新規・再開／開始フェーズ／テンプレ有無／banana 利用可否 など）
- 発明カテゴリの選択（システム／方法／プログラム。`multiSelect`）
- 独立項の広さ（広く取る／限定する）など二者択一のトレードオフ
- 画面種別・レイアウト・デザイントーンなど候補を列挙できる設計判断（Phase 2）
- 図面の枚数・規模感（中規模10〜15枚／大規模20〜30枚 等。Phase 4）
- どの実施形態バリエーション・どの公知例を深掘りするか（`multiSelect`）
- **各フェーズ末尾のレビュー承認**（「次フェーズに進む／さらに修正する」）
- 既存成果物（`output/*` 等）の上書き可否

### 使い方の原則

- **選択肢が列挙できる問いは必ず `AskUserQuestion`。** 1問につき2〜4個の具体的な選択肢を出し、推奨案があれば先頭に置いてラベル末尾に「（推奨）」を付す。
- **トレードオフは選択肢の `description` に書く**（例：「広く取る＝権利範囲は広いが拒絶リスク増／限定する＝通りやすいが狭い」）。発明者が判断材料を持てるようにする。
- 互いに排他でない選択（カテゴリ展開、採用する実施形態、深掘りする公知例など）は `multiSelect: true`。
- 関連する判断は **1回の `AskUserQuestion` に最大4問までまとめて** 聞き、一問一答で往復しない（各フェーズの「3〜5項目をまとめて聞く」方針と整合）。
- **純粋な自由記述が必要な問い**（アイデアそのものの説明、固有の数値・固有名詞など候補を出せないもの）は無理に選択肢化しない。ただしその場合も、まず Claude がドラフト案を提示し、発明者は修正だけ返せばよい形にする。
- 選択肢に無い答えは発明者が「その他」で自由入力できる（ツールが自動付与）。これを前提に選択肢は代表例で構わない。

## 重要な原則

1. **対話的に進める。** Phase 1で発明者からヒアリングする項目は決まっている（`phases/1-expand.md`参照）。勝手に埋めず、不足は必ず聞く。**選べる問いは上記「対話ルール」に従い `AskUserQuestion` で選択肢を出す。**
...
ユーザーがスキルを呼んだら、以下を確認してから着手する。**この確認は `AskUserQuestion` で行う**（案件名のような自由記述項目を含め、選べるものは選択肢化し、関連項目は最大4問にまとめて1回で聞く）：
