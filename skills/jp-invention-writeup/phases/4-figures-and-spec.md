...
規模感（中規模10〜15枚／大規模20〜30枚 等）や、図面リスト案で進めてよいかは `AskUserQuestion` で選ばせ、追加・削除したい図があれば「その他」で自由入力してもらう。
...
### 5.5. 用語の定義を書く（造語・非標準用語がある場合は必須）

請求項・本文で使う語のうち、**造語・非標準用語・本明細書で特有の意味を持たせる語**を `work/term-definitions.md` に定義する。`templates/term-definitions.template.md` を雛型にする。

明細書では、こうした用語の意義は **〔発明を実施するための形態〕の冒頭** にまとめて記載するのが定石。読み手（審査官・他社・裁判所）が請求項を一義的に解釈できるようにし、サポート要件・明確性要件の担保にもなる。

判断基準：
- **定義する**：その分野でまだ一般的でない語、複数の意味に取れる語、発明者が独自の意味で使う語（例：本件の「スキル」「エージェント」「抽象化キー」「戦略」「フレッシュネス確認」など）
- **定義しない**：一般的な技術用語（CPU、データベース、言語モデル等）

書き方（`references/jpo-style.md`「〔用語の定義〕」参照）：
- 文型は「本明細書において『○○』とは、〜をいう。」で統一
- **広めに定義する**（「〜を含む」「〜等」を使い、実施例に縛られない）。狭く定義すると請求項の語まで狭く解釈され、権利範囲を自ら狭める
- 請求項で使う主要語との不整合がないか確認（同じ語は同じ意味で使う）
...
`--definitions` は任意。`work/term-definitions.md` があれば〔発明を実施するための形態〕冒頭に〔用語の定義〕として挿入される（無ければ従来どおりスキップ）。
...
8. 【発明を実施するための形態】
   - 〔用語の定義〕（term-definitions.md から。造語・非標準用語の意義）
   - 各図の説明（figure-descriptions から）
9. 【符号の説明】
